...
public class spriteAC : MonoBehaviour {
	...
	public bool manualIndex = false;
	public int _index = 0;                         // Keeps track of the current frame 
	...
	public void ChangeMaterial(Material newMaterial, bool newInstance = false)
	{
		if (newInstance)
		{
			...
			if (_hasMaterialInstance)
				Object.Destroy( GetComponent<MeshRenderer>().sharedMaterial);

			// create the new material
			_materialInstance = new Material(newMaterial);

			// Assign it to the renderer
			GetComponent<MeshRenderer>().sharedMaterial = _materialInstance;
		...
		GetComponent<MeshRenderer>().sharedMaterial.SetTextureScale("_MainTex", _textureSize);
	}
	...
	private void Awake()
	{
		...
		ChangeMaterial( GetComponent<MeshRenderer>().sharedMaterial, _newMaterialInstance);
	}
	...
	private void Update(){
		...
		if (elapsedT > maxT & _disableUponCompletion)
			Destroy (gameObject);
	}
	*/
